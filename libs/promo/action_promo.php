<?php
class ActionPromo extends Object {
	var $label = "Action (Deprecated)";
	
	function isAvailable(){
		$this->ShopAction = ClassRegistry::init('Shop.ShopAction'); 
		$actions = $this->ShopAction->find('count',array('conditions'=>array('status'=>'checkPromo')));
		return $actions>0;
	}
	...
	function editForm($view){
		...
		',array('inline'=>false));
		echo $view->Form->input('action_id',array('empty'=>__('None',true)));
		?>
